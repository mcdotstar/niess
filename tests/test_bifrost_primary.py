...
def test_primary_parameters():
    ...
    expected_sections = {
        ...
        unit_27_expanding
        unit_28_expanding
        unit_28_exit_window""").splitlines(),
        'straight': dedent("""\
    ...
    for section, expected_names in expected_sections.items():
        section_names = list(parameters[section].keys())
        for name, ex in zip(section_names, expected_names, strict=True):
            assert name == ex, f'{section}: {name} != {ex}'
        assert len(section_names) == len(expected_names)
